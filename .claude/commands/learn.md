...
### Step 5b: Update skills-index.md

After saving all approved skills (at least one written), update `.claude/docs/skills-index.md`:

1. Read the current file.
2. Find or create the `## Learned Skills (\`skills/learned/\`)` section. If missing, append it before the `## Writing New Skills` section:
   ```markdown
   ## Learned Skills (`skills/learned/`)

   Generated by `/learn`. Project-specific patterns extracted from completed implementation work:

   | Skill | Covers | Confidence |
   |-------|--------|-----------|
   ```
3. For each saved skill:
   - If already listed (match by slug), update the `Covers` and `Confidence` cells.
   - If new, append a row: `| \`<slug>\` | <description from frontmatter> | low/medium/high |`
4. Write the updated file. Print: `skills-index.md updated — <N> rows added/updated`.

### Step 6: Generate PROMPTS.md

After saving all approved skills, generate a `PROMPTS.md` file documenting the Claude Code workflow that produced the implementation.

**When to generate:** Only when at least one skill was saved (don't generate for zero-skill runs).

**Before generating:** Ask the user for the module name and any workflow notes — they have context the agent lacks.

**Where to save:**
- If a specific module was the focus: `[module folder]/PROMPTS.md`
- Otherwise: `docs/PROMPTS-[feature]-[date].md`

**Show a preview and ask for approval before saving.**
