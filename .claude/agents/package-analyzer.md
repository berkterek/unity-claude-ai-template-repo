...
|----------|----------|--------|
| **MUST-FIX** | `static.*Instance`, `Input.GetKey/Axis/Button` | Blocking hooks fire when Claude writes wrapper code that touches these |
...
For each finding record: `{ "severity": "MUST-FIX|SHOULD-FIX|CONSIDER", "pattern": "<what was found>", "file": "<relative path>:<line>", "hook": "<hook name or rule>", "fix": "<one-line recommendation>" }`.
...
> Generated by `/discover`. Patterns in this package that conflict with project rules.
> Fix these before or while writing wrapper code — blocking hooks will fire otherwise.

## MUST-FIX

| Pattern | Location | Hook | Recommended fix |
|---------|----------|------|-----------------|
| `FooManager.Instance` | FooManager.cs:42 | check-vcontainer-singleton | Register as IFooService in AppScope installer |
| `Input.GetAxis("Horizontal")` | FooInput.cs:18 | check-input-system | Replace with InputView + PlayerControls.inputactions |

## SHOULD-FIX

| Pattern | Location | Rule | Recommended fix |
|---------|----------|------|-----------------|
| `StartCoroutine(FooRoutine())` | FooController.cs:55 | UniTask rule | Wrap in `async UniTask FooAsync(CancellationToken ct)` |
| `Resources.Load<GameObject>(...)` | FooLoader.cs:30 | Addressables rule | Use `Addressables.LoadAssetAsync` + `AssetAddresses` constants |
| `FindObjectOfType<FooManager>()` | FooHelper.cs:12 | VContainer rule | Inject `IFooService` via constructor or `[Inject]` |

## CONSIDER

| Pattern | Location | Recommendation |
|---------|----------|---------------|
| `GetComponent` in Awake | FooView.cs:8 | Assign via `[SerializeField]` in Inspector if component exists at edit time |
```
