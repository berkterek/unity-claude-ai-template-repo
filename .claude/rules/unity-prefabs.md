...
**Nor is `nested prefab instances skipped: N` a pass for those N objects.** A nested prefab instance serializes as *stripped* blocks — no `m_Name`, no component list, because its components live in the referenced asset — so condition 2 above is a vacuum, not a match, and the script exempts them and prints the count. Until 2026-09-10 it did not: a real `CanvasHUD.prefab` reported `['?', '?']` with an empty component set and exited 1, on two instances whose `m_SourcePrefab` GUIDs actually differed. The verdict was backwards even when they match, since repeated instances under a layout parent is exactly what this card *prescribes*. What is genuinely unchecked is duplication **between** nested prefabs; compare `m_SourcePrefab` by hand when the count is non-zero. Inline hand-duplicated siblings are unaffected — they still carry their components in the file and are still caught.
