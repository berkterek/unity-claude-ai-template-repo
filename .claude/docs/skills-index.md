...
| Skill | Covers |
|-------|--------|
| `knowledge-graph-hybrid` | Routes 4 call-graph queries to `graph-mcp-server.py` (via `graph_bfs_core.py`) with `graph-traversal.py` fallback; 11 Unity-semantic queries stay on graph.json | `.claude/skills/core/knowledge-graph-hybrid.md` |
