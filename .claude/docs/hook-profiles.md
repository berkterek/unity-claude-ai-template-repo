...
| `check-domain-folder-structure.sh` | **PreToolUse.** Blocks layer names (`Services/`, `Views/`, `Providers/`, …) and catch-alls (`Core/`, `Generals/`) as the first folder under `Games/Abstracts\|Concretes/`, and `.cs` files with no domain folder. Never inspects below the domain. Disable: `DISABLE_HOOK_CHECK_DOMAIN_FOLDER_STRUCTURE=1` |
