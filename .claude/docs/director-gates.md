...
| **Deleted** | **At Completion only, with `gate-cleared` — never when the gated coder returns.** Deleting it on coder completion deadlocks the first fix pass deterministically: the validator and reviewer fix passes (`/implement` Steps 2.5, 3, 3.6, 3.7; `/fix`; `/orchestrate`'s Ralph loop) all spawn a coder-class agent again, `guard-sparc-approved.sh` exits 2 for every one of them while the file is absent, and none of those steps re-opens the gate — so the pipeline blocks itself on an approval the human already gave, with a block message indistinguishable from "the gate was never shown". The approval does not need that deletion to be bounded: `guard-sparc-approved.sh` calls `unity_gate_cleared_valid "sparc-approved"`, so the same 45-minute `UNITY_GATE_TTL` as `gate-cleared` applies, plus `session-restore.sh`'s SessionStart clear. Same shape as the compaction-revokes-`gate-cleared` bug: the file already had a bound, and the second, eager teardown was the defect. Fixed in the commands by `6f62324`; this row lagged that commit and re-introduced the deadlock for any Director that read the table instead of the command. |
