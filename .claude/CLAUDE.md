...
|------|----------|--------------|-----------------|
| `SCOPE_GATE` | `/implement`, `/fix`, `/migrate`, `/scene-setup` | After complexity scoring, before any agent spawns | Confirm scope matches intent — type `go` or redirect |
| `ARCHITECTURE_GATE` | `/implement`, `/scene-setup` | When new module folder detected (+0.3 complexity signal) | Approve proposed module structure (interface/service/installer) |
| `BREAKING_GATE` | `/fix` (>3 files), `/migrate` (>5 files) | After affected files identified | Confirm wide-blast-radius change is intentional |
| `QUALITY_GATE` | All pipeline commands | After reviewer returns CHANGES NEEDED | Choose: `fix` / `skip` / `stop` |
| `COMMIT_GATE` | `/implement`, `/fix`, `/migrate`, `/scene-setup` | After all verification, immediately before committer | Final sign-off on staged files — type `go` or `stop` |
...
Small packages (< 10 prefabs) use a single `SKILL.md`. Medium packages add `prefabs.md`. Large packages (50+) use the full split. Pre-built static skills that came with the template remain in `skills/plugins/`.
