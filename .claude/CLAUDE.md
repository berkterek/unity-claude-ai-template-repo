...
- `.claude/graph/graph.json` is generated — never edit by hand. Use `/build-knowledge-graph` to refresh. **Partition architecture (since v1.3.0):** `scenes[]` and `prefabs[]` are stored in sibling files `scenes.json` and `prefabs.json` (same dir). `graph.json` holds `{"$partition": "scenes.json"}` refs. All three files are generated and committed together. **Schema is v1.5.0**, which added scope-parent resolution fields; v1.4.0 added `extraction_version`.
- **`extraction_version` vs `schema_version` — they answer different questions, do not conflate them.** `schema_version` tracks the document's *shape*; `extraction_version` tracks *what the values mean*. A change that makes the same input file produce a different record (e.g. which type lands in `registrations[].type`) bumps `EXTRACTION_VERSION` in `graph-builder.py` and **not** `schema_version`. Both move together only when a change does both at once — v1.5.0's scope-parent work added fields *and* changed what a record says, so it bumped `EXTRACTION_VERSION` 2→3 and `schema_version` 1.4.0→1.5.0. That is the exception; assume one, not both. The builder reads the stored value back on `--incremental` runs and promotes one run to `--full` on a mismatch, absent, or corrupt value — self-clearing, non-fatal, reason on stderr. This exists because `generator` and `schema_version` are write-only and staleness was judged purely on a 24h clock, so old wrong records survived indefinitely while the graph reported itself fresh.
...
- **Registration records name the concrete type, with the interface in `as`.** `RegisterInstance<IFoo>(new Foo())` → `type: "Foo"`, `as: "IFoo"`; `/knowledge-graph registrations` resolves through either name. **Known limit:** `.AsImplementedInterfaces()` — the dominant idiom in this project, mandated by `rules/bootstrap-pattern.md` — stores that literal string as a **placeholder**, not an interface name, so `registrations IFoo` will not find those registrations. Use `/knowledge-graph implementers` for that until it is resolved properly.
...
> **`subagent-depth` leaks — treat it as a hint, never as fact.** The pair only balances when every increment gets a matching Stop; an agent that errors, is interrupted, or is still running when the session ends leaves the count permanently high (measured once: 340 Start vs 319 Stop records, counter at 12 with no agent running). `session-restore.sh` resets it at SessionStart. Each consumer resolves a doubtful count toward **enforcing its own rule**, so the two staleness directions are deliberately opposite: `guard-pipeline-direct-work.sh` allows on depth > 0 and therefore downgrades a stale count to 0; the deny-then-allow gates (`gateguard.sh`, `guard-critical-files.sh`, `check-config-protection.sh`) *pass* on 0 and therefore never downgrade — a timeout there would release a long-running subagent through the gate. If a leaked count locks the Director out: `echo 0 > "$CLAUDE_PROJECT_DIR"/.claude/state/subagent-depth`.
