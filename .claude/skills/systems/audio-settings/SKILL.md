---
name: audio-settings
description: Use when implementing audio settings UI (volume sliders, mute toggles), persisting audio preferences via PlayerPrefs or save system, or wiring AudioConfiguration ScriptableObject with VContainer for runtime volume control.
globs: ["**/*AudioConfig*.cs", "**/*AudioSettings*.cs", "**/*AudioService*.cs", "**/*SoundSettings*.cs"]
---
...
public static class AudioModule
{
    public static void Install(IContainerBuilder builder, AudioConfiguration config, AudioMixer mixer)
    {
        ...
        if (mixer == null)
        {
            Debug.LogError("[AudioModule] AudioMixer missing.");
            return;
        }
...
Called from `AppModules.Install()` with the configs from `ConfigCatalog`. Guards use `Debug.LogError` + `return`, never `throw` — see `rules/bootstrap-pattern.md`.
