---
name: dump
description: Save current session notes, decisions, and context to .claude/logs/ as a markdown file for future reference
user-invocable: true
model-tier: light
---
...
Saves a summary of the current session to `.claude/logs/YYYY-MM-DD-<topic>.md`.

## Steps

1. Ask the user (or infer from conversation): what was the main topic of this session?
2. Create `.claude/logs/` if it doesn't exist
...
- `.claude/logs/` should be in `.gitignore` unless the user wants to commit session logs
