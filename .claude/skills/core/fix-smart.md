---
name: fix-smart
description: Codex-first bug fix pipeline — Codex analyzes and fixes autonomously, Claude acts as architecture reviewer. Use when /fix and /fix-deep have failed, or as the primary fix command for stubborn/complex bugs.
user-invocable: true
model-tier: standard
---
...
Check that `codex:rescue` skill is available. If not, stop and tell the user to run `/codex:setup`.
...
Invoke `codex:rescue` with the full context package from Step 1. Structure the prompt as:
