...
| `guard-reviewer-order` (PreToolUse) | `unity-reviewer` spawn if Codex CLI is installed but `codex:codex-rescue` has not reviewed the current pipeline pass — uses `gate-cleared` timestamp to detect stale markers from previous runs; bypass: `touch .claude/state/codex-reviewed` |
...
Small packages (< 10 prefabs) use a single `SKILL.md` (with inline compliance section if violations found). Medium packages add `prefabs.md`. Large packages (50+) use the full split.

**Compliance severities:** `MUST-FIX` (blocking hooks will fire — e.g. singleton → VContainer, legacy Input), `SHOULD-FIX` (architecture rules — e.g. coroutines → UniTask, `Resources.Load` → Addressables), `CONSIDER` (good practice — e.g. `GetComponent` in Awake).
