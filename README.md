...
Skills live under `.claude/skills/` and are loaded automatically by commands. They are read-only reference files that inform Claude's decisions — they do not execute code. The `/learn` command writes project-specific patterns to `skills/learned/` and automatically adds entries to `skills-index.md` so they are visible in future sessions. The `/discover --write` command writes third-party package skills to `skills/third-party/<pkg>/` and also updates `skills-index.md`.
