...
| `check-no-monobehaviour-in-services` | `class FooService : MonoBehaviour/ScriptableObject` in `_Framework/` / `Games/Abstracts/` / `Games/Concretes/` | Make it a Provider, View, Controller, or Manager instead (needs an own `[SerializeField]` or a Unity lifecycle callback) — `using UnityEngine` for math types is allowed |
