...
| `guard-pipeline-direct-work` (PreToolUse Edit\|MultiEdit\|Write\|Bash) | Blocks direct `Edit`/`Write` to `_GameFolders/Scripts/**/*.cs` and direct `git commit` while a Director Gate is open (`gate-cleared` exists) but no subagent is currently running (`subagent-depth` == 0) — closes the "gate was shown but pipeline agent was never spawned" loophole. The depth counter can still leak (an agent interrupted, erroring, or still running when the session ends never gets its matching decrement — see `agent-start-log`), so a count untouched for 15 min is read as 0 (fails toward enforcing). **2026-08-29:** the increment/decrement itself was a read-modify-write race — parallel `Agent` spawns could all read the same stale value and lose updates (4 parallel spawns measured landing at depth `1` instead of `4`), which then made this hook treat a genuinely running subagent's first `Write` as if none existed. Fixed with an `mkdir`-based lock (`unity_subagent_depth_lock`/`_unlock` in `hooks/_lib.sh`) around both writers — the on-disk counter shape and every reader are unchanged. Escape valve: `.claude/state/pipeline-override` for explicit user-approved bypasses |
