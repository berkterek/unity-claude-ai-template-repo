...
> **`/fix` vs `/fix-deep`:** Use `/fix` when the stack trace clearly points to the root cause. Use `/fix-deep` for logic bugs, "sometimes happens" issues, or any case where the root cause is uncertain.
...
| `package-analyzer` | Read-only analyst — walks `Packages/manifest.json` and resolved package directories, emits multi-file skill drafts under `.claude/skills/third-party/<pkg>/`. Detects prefabs and maps them to `_GameFolders/Prefabs/<Category>/` destinations |
...
**VContainer** — Install via Unity Package Manager (add by git URL)

**UniTask** — Install via Unity Package Manager (add by git URL)
...
**Addressables** _(only if Addressables=yes)_
...
**Scenes** (do not edit scene YAML directly — use the Unity Editor or the Unity MCP integration)
