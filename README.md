...
|---------|------------|-------------|
| `/game-idea` | Manual — single step | Refines a raw idea into a GDD — surfaces assumptions, defines scope, creates a "Not Doing" list, and defines **Milestones** (mandatory Category 6: *"what do we need to SEE on screen?"* → M0 First Playable + smoke-test checklist + per-milestone evidence environment and what it cannot show + stub policy + an optional scope budget with its consequence + a dated ordering rationale) |
...
| `gateguard` (PreToolUse) | Stage 1 = the unread-file check above; Guard 2 = the fact gate. Guard 2 is released when the open plan covers the file. The fact demands themselves live at plan time now: `.claude/scripts/validate-plan-facts.sh` runs BLOCKING in `/create-plan`, `/plan-module` and `/orchestrate` (see Plan-Time Validation for where in each — the plan must be on disk first), sharing `hooks/lib-gateguard-facts.sh` with this hook — one library, two callers, no cache and no receipt file. This is what makes the `strict` profile usable inside `/orchestrate`; before it, `guard-pipeline-direct-work` blocked the Director while this blocked the subagent, so with a gate open nobody could write a `.cs` at all. For `_GameFolders/Scripts/**.cs` the block message deliberately does **not** offer the deny-then-allow retry: that branch is Director-only, and `guard-pipeline-direct-work` stops the Director first, so it is unreachable there (this hook is strict-level, so it is the `strict` profile — and `/orchestrate` — that this describes; under `standard` gateguard does not run at all) — it names the two doors that do exist (declare the path in a `tasks.md` task, or spawn the pipeline agent) instead. Outside that path pattern the retry is real and is still offered |
| `guard-pipeline-direct-work` (PreToolUse Edit\|MultiEdit\|Write\|Bash) | Blocks direct `Edit`/`Write` to `_GameFolders/Scripts/**/*.cs` and direct `git commit` while a Director Gate is open (`gate-cleared` exists) but no subagent is currently running (`subagent-depth` == 0, read via `unity_subagent_depth()`) — closes the "gate was shown but pipeline agent was never spawned" loophole. The depth counter can still leak (an agent interrupted, erroring, or still running when the session ends never gets its matching decrement — see `agent-start-log`), so a count untouched for 15 min is read as 0 (fails toward enforcing). **2026-08-29 (partial fix):** the increment/decrement itself was a read-modify-write race — parallel `Agent` spawns could all read the same stale value and lose updates (4 parallel spawns measured landing at depth `1` instead of `4`). Fixed with an `mkdir`-based lock (`unity_subagent_depth_lock`/`_unlock` in `hooks/_lib.sh`) around both writers. **This did not fix real coder/tester subagents getting blocked during `/orchestrate`** — their Start/Stop log entries were sequential, not overlapping, so no race was involved. **2026-08-29 (root cause #1):** `_lib.sh`'s `_resolve_state_dir()` resolved the state directory via `git rev-parse --show-toplevel`, which depends on the caller's cwd. A subagent's own tool-execution context isn't guaranteed to run from the repo root; when it doesn't, `git rev-parse` there resolves elsewhere (or fails) and the function silently falls back to `/tmp/unity-claude-hooks` — an always-fresh, always-zero directory the main session never touches. Fixed by preferring `$CLAUDE_PROJECT_DIR` and falling back to `git rev-parse` only when it's unset. **2026-08-29 (root cause #2, found after #1 was already fixed):** `PostToolUse:Agent` (which decrements depth) fires on the Agent tool's *async dispatch acknowledgement* in this harness, not on the subagent's actual completion — measured `duration_approx_s` of 1-3s against a real `usage.duration_ms` of 45-54s for the same calls. The decrement was happening ~50 seconds before the subagent actually finished, including through the moment it called `Write`. Fixed by deferring the decrement: `agent-stop-log.sh` now calls `unity_subagent_schedule_decrement()` with a `not_before` epoch anchored on the matched Start timestamp plus `UNITY_SUBAGENT_STOP_GRACE_SECONDS` (default 180s), and `unity_subagent_depth()` applies any matured pending decrement lazily on read (`unity_subagent_apply_pending_decrements()`) — there's no background timer in a stateless hook system. A heuristic, not an exact signal (no reliable "subagent truly finished" event exists here — native Stop is unreliable, `TaskCompleted` tracks todo items, not `Agent` dispatches), but it fails in the safe direction for this hook: a late decrement only over-permits a bounded window after real completion, never under-permits mid-run. Escape valve: `.claude/state/pipeline-override` for explicit user-approved bypasses |
...
| `check-enum-byte-base` | `enum` without `: byte` base inside `IComponentData` or `IEvent` structs — use `: ushort` if 255+ values needed. Feature-gated on `ecs`; the gate file path is resolved via `$CLAUDE_PROJECT_DIR` first (2026-08-29 fix — a bare `git rev-parse` there mis-resolved inside a subagent's cwd and silently no-op'd this check even when ECS was genuinely enabled) |
| `check-duplicate-symbol` | New file's top-level symbol already exists elsewhere in the graph. Feature-gated on `graph` + a present `graph.json`; both gate-file paths use the same `$CLAUDE_PROJECT_DIR`-first fix as `check-enum-byte-base` (2026-08-29) |
| `block-graph-direct-read` (PreToolUse Read) | Direct `Read` of `graph.json`, `scenes.json`, or `prefabs.json` when `hybrid_graph: true` — use `/knowledge-graph` subcommands or `mcp__graph_mcp__*` tools instead. **2026-08-29:** `PROJECT_ROOT` resolution now prefers `$CLAUDE_PROJECT_DIR` — a bare `git rev-parse` came back empty inside a subagent's cwd and exited this guard entirely, silently allowing the direct Read it exists to block |
| `check-domain-folder-structure` (PreToolUse Edit\|Write) | **Fail-closed, all file types** (an `.asmdef` is how an illegal folder is born). Any first segment under `Scripts/` outside `Games\|Tests\|Editors`; any first segment under `Scripts/Games/` outside `Abstracts\|Concretes\|Ecs`; a file at the root of `Scripts/`; layer names (`Service(s)/`, `Provider(s)/`, `Controller(s)/`, `View(s)/`, `Manager(s)/`, `Interface(s)/`, `Config(s)/`) and catch-alls (`Core/`, `General(s)/`) as the **first** folder under `Games/Abstracts\|Concretes/`; a `.cs` file with no domain folder at all. Never inspects below the domain — `Players/Services/` is legal. Rule logic lives in `hooks/lib-path-rules.sh`, shared with the plan-time `scripts/validate-plan-paths.sh`; exceptions are declared in `.claude/path-allowlist.txt`. `Tests/`, `Editor/` are out of scope. **2026-08-29:** `lib-path-rules.sh`'s allowlist-file lookup now prefers `$CLAUDE_PROJECT_DIR` over a bare `git rev-parse` — the latter fell back to `.` inside a subagent's cwd, silently un-matching a real, reviewed `path-allowlist.txt` entry |
...
| `agent-start-log` (SubagentStart) | Appends spawn record to `.claude/state/subagent-log.jsonl` — `agent_type`, `description`, `session_id`, `started_at`, `is_retry`. Advisory only (exit 2 not honoured on SubagentStart). **Retry dedup:** if an unmatched (no `SubagentStop` yet) Start already exists for the same `session_id`+`description`, this Start is logged with `is_retry:true` and does **not** increment `subagent-depth` — measured cause of the dominant depth-leak pattern: the Agent tool's internal retry-on-transient-error path fires a new `PreToolUse/Agent` per attempt but only ever resolves to one `PostToolUse` Stop (N Starts, 1 Stop, same description, seconds-to-minutes apart). **Parallel-dispatch race (fixed 2026-08-29):** the increment itself (`read subagent-depth → +1 → write`) was not atomic — concurrent `Agent` spawns could all read the same value before any write landed, losing updates. `agent-start-log.sh` and `agent-stop-log.sh` now wrap their read-modify-write in an `mkdir`-based mutex (`unity_subagent_depth_lock`/`_unlock`, `hooks/_lib.sh`); verified in isolation with 4 parallel spawns landing at depth `4` (previously `1`) and 4 parallel stops returning to `0`. **This lock did not fix real `/orchestrate` subagents being blocked** — that symptom's own Start/Stop entries were sequential, so no race applied; see `_resolve_state_dir()` fix below and in `guard-pipeline-direct-work` above for the actual cause (cwd-dependent `git rev-parse` sending a subagent's hook invocation to a different/`/tmp` state directory than the main session's). |
...
>
> **Tester isolation:** The `tester` step in `/implement`, `/fix`, `/orchestrate`, and `/migrate` runs as an isolated `claude` subagent. It receives a clean context window and reads `tester.md` + `testing.md` directly — preventing implementation context from leaking into test decisions. The `committer` step in these pipelines runs inline for the opposite reason: a subagent would see the diff but not the conversation, and a commit body worth reading explains *why* — which lives in the conversation, not the diff. (`/create-plan` and `/update-plan` do spawn it as a subagent — there the plan file already carries that context.)
...
Both take either a single file or a directory to walk, but **they do not walk the same corpus** — `validate-plan-facts.sh` collects only `tasks.md` and skips `_templates/`, while `validate-plan-paths.sh` collects every `*.md` it finds. Point them at a module plan folder (`docs/modules/NN-name/`), which is what the three commands pass; pointing the path validator at a broad root makes it read illustrative `WRONG:` paths out of prose and report them as violations.

Why `_templates/` is skipped: `docs/modules/_templates/tasks.md` is a form, and its `[Domain]` placeholders own no folder and no `.asmdef`, so walking into it would report violations for paths nobody intends to write. Naming that file explicitly still scans it — naming it is deliberate, walking into it is not. A walk that finds no `tasks.md` at all exits **1** with `no tasks.md to scan under: <path>`, which is a different condition from the one below.
...
> **The plan corpus is resolved from `$CLAUDE_PROJECT_DIR`, not the caller's cwd — and it says so out loud when it cannot be resolved.** `UNITY_PLAN_ROOT` used to default to a bare relative `docs`, which resolves against whoever called the hook. A subagent's tool-execution cwd is not guaranteed to be the repo root, so `find docs -name tasks.md` matched nothing there and every path came back "no task declares this" — about a plan that was never opened. Because it depends on cwd rather than content, it fails *inconsistently*: one file passes on the first try and the next blocks three times, with the same plan and the same rules. Measured 2026-09-07 against a real downstream plan: `COVERED` from the repo root, `NOT COVERED` from anywhere else. Fixed to prefer `$CLAUDE_PROJECT_DIR/docs` (the same fix `_lib.sh` and `lib-path-rules.sh` took on 2026-08-29 — this was the third instance of that one bug class), with an explicit `UNITY_PLAN_ROOT` still winning so tests and the two scripts are unaffected. `unity_plan_root_status()` now separates the two verdicts that used to look identical: an unreachable corpus prints `PLAN ROOT NOT FOUND: <path>` and states that this is **not** a claim about the plan's contents. If you see that line, check the root before editing any `tasks.md`.
>
> **The rename detector reads a task's TITLE LINE, and an arrow is not a rename signal (fixed 2026-09-09).** The `FormerlySerializedAs:` demand — a real rule, guarding against every configured value in every scene and prefab silently resetting to default — used to be triggered by a raw `grep -qiE 'rename|…|→'` over the **whole** task body. Two independent false positives came out of that, and the second is not an edge case at all: `unity_find_task_line` captures every sub-bullet, and this project's own task template mandates a `Wiring:` field shaped ``Wiring: registered in `XModule.cs` via `Install()` → `Register<X>()…` `` — so the arrow alternative matched the template's own required field, and **every edit task written from the template against a file containing `[SerializeField]` demanded a declaration for a rename that was not happening**. The first false positive was narrower and is what surfaced it: an acceptance criterion reading *"level_1.npy temporarily renamed and Import is pressed"* describes moving a data file to exercise an error path, not renaming a C# field. Both are closed by narrowing the search to the title line and dropping `→`: a field rename is by definition what the task *does*, which is the title, and the arrow is ordinary prose punctuation throughout this repo.
>
> **Why it stayed invisible for so long, and why that matters more than the fix.** The rule fires only at `mode=edit`, and plan-time validation runs before any target exists — so at plan time every task is `new` and the branch is never reached. It fires for the first time during the fix pass, *after* the file is on disk, which means a newly created file became uneditable by its own creation: same shape as the `PreToolUse` effective-content bug documented in `CLAUDE.md`, one layer up. And the branch was **tested** — four `lib-gateguard-facts.bats` cases covered it, every one of them titled with a genuine `rename _speed to _moveSpeed`. The tests were shaped to the bug, exactly like `reg.4`'s `ProbeInstaller.cs` fixture and the 29 tests that always set `UNITY_PLAN_ROOT` explicitly. Three cases now pin the false-positive side (arrow in `Wiring:`, `renamed` in an acceptance criterion) alongside a regression guard that a real title-line rename is still blocked.
>
> Two things generalise past the fix. The default branch had **no test coverage at all** — all 29 `lib-gateguard-facts.bats` tests set `UNITY_PLAN_ROOT` explicitly — so a green suite said nothing about the path that actually ran in production; when auditing for this bug class, look for the default nobody exercises. And the failure was silent in the worst direction: coverage is the only door `gateguard.sh` leaves open to a subagent, while `guard-pipeline-direct-work.sh` simultaneously closes the Director's, so a merely-different cwd deadlocked the whole pipeline and reported it as a planning defect.
